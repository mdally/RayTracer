...
inline
int myroundf(float x){
   return x >= 0.0f ? (int)floorf(x + 0.5f) : (int)ceilf(x - 0.5f);
}

enum VertexType { LEFT, RIGHT, VERTICAL, NOT_SET };
...
private:
	...
	//simple method to print the current mask to the console for testing purposes
	void printMask();

	void scanConvertLine(LineSegment& line, std::vector<std::vector<int>>& boundaries);

	void safePutPixel(int x, int y, Color c);

	void fillCircleOctants(int x, int y, int center, std::vector<std::vector<int>>& boundaries);

	void printCircleBoundaries(int center, int radius, std::vector<std::vector<int>>& boundaries);
